    ...
    <div class="row mb-2">
        <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">แก้ไขรายการเบิกวัตถุดิบ</li>
            </ol>
        </div><!-- /.col -->
    ...
    <div class="card-body">
        <form id="form_valid">
            <div class="row">
                    ...
                    <select name="" id="material_id" class="form-control">
                        ...
                        @foreach ( $materials as $material )
                            @if ($material->record_status === 1)
                                ...
                                <option value="{{$material->id}}" data-name="{{$material->name}} ">{{$material->name}} : จำนวน {{ number_format($material->stockremain / 1000) }} kg.</option>
                            @endif
                        @endforeach
                    </select>
                ...
                <div class="col-sm-12 col-md-4 col-lg-4 col-xl-4 ">
                    <button type="button" class="btn btn-save ogn-stock-green text-white add-to-table2 float-right position-absolute" style="bottom: 0; right: 8px;"><em class="fas fa-plus-circle"></em>  เพิ่มเข้ารายการเบิก</button>
                </div>
            </div>
        </form>
    </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.forEach(element => {
                    ...
                    row +=
                `<tr class="materialrowold list_material">
                ...
                <td><span class="mat_weight mat_hide ">${element.weight}</span>${weight} ${wunit}</td>
                </tr>`
    ...
    var material = []
        ...
        function check_duplicate_numberlot(){
            ...
            var status = true;
            if (lot != "" || null) {
                $('.list_material').each(function(index,value){
        ...
        $('.add-to-table2').on('click', function(){
            ...
            var text_weight = "";
                ...
                if (parseFloat(total_weight) > parseFloat(weight_stock * 1000)) {
                    ...
                    .to$()
                    .addClass( 'materialrow list_material' )
                }
        ...
        $('#create_receive_material2').on('click', function() {
            ...
            $('.materialrowold').each(function(index,value){
                formdata_receive.append('materialold['+index+'][material_lot_id]',$(value).find('.mat_lot_id').text())
                formdata_receive.append('materialold['+index+'][material_id]',$(value).find('.mat_id').text())
                formdata_receive.append('materialold['+index+'][weight]',$(value).find('.mat_weight').text())
                formdata_receive.append('materialold['+index+'][date]',$(value).find('.mat_date').text())
                numrowold++
            })
                ...
                if (rows_material > 0) {
                    Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $.ajax({
                                        type: "post",
                                        url: "{{route('api.v1.requsition.material.edit')}}",
