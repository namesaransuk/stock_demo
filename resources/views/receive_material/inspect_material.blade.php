...
@section('content')
    <div class="card">
        ...
        <form id="formInspect" action="{{route('receive.material.check.save')}}" method="POST">
            @csrf
        <div class="card-body">
            <div class="row">
                <div class="col-sm-4">
                    <input type="hidden" name="id" id="id" value="{{$mat_lot_detail->id}}">
                ...
                <div class="col-sm-8 ">
                    <label class="label-control" for="">Supplier</label>
                    <input class="form-control" type="text" name="" id="" value="{{$brand_vendor_name->brand}}" readonly>
                </div>
                ...
                <div class="col-sm-2 pt-2">
                    ...
                    <input class="form-control" type="text" name="" id="" value="{{$mat_lot_detail->mfg}}" readonly>
                </div>
                <div class="col-sm-2 pt-2">
                    <label class="label-control" for="">EXP.</label>
                    <input class="form-control" type="text" name="" id="" value="{{$mat_lot_detail->exp}}" readonly>
                </div>
            ...
            <div class="row mb-5">
                <div class="col-12">
                    ...
                    <div style="text-align: right;">
                        <a class="btn ogn-stock-grey text-black btn-back btn-back"><em class="fas fa-arrow-left"></em> ย้อนกลับ</a>
...
@section('js')
   <script>
      ...
      $(function(){
        ...
        var material_lot_id = $('#id').val();
        $.ajax({
            type: "post",
            url: "{{route('api.v1.receive.material.quality.check.detail')}}",
            data: {"id" : material_lot_id},
            dataType: "JSON",
        ...
        let check_receive_table = $('#receive_material_table_check').DataTable({
            fixedColumns: {
                leftColumns: 2
            },
            scrollY:        300,
        ...
        $('#receive_material_table_check tbody').on('click', '.list_cancel', function() {
        ...
        $('#inspect_template_id').on('change',function(){
            ...
            $.ajax({
                type: "post",
                url: "{{route('api.v1.receive.material.get.template.detail')}}",
      ...
      function loadEvalutionTopic(response) {
          ...
          $('#inspect_template_id').prop('disabled', 'disabled')
        //   console.log("Eval")
        //   console.log(response)
          $(".append_next").remove();
          ...
          $.each(response, function(key, topic){
              if ( topic.ins_type==="1") {
                  let tds = ``;
                  // console.log(topic.)
                  let _detail_len = topic.material_inspect_details.length-1;
                  $.each(topic.material_inspect_details, function (key2, dtl) {
                      ...
                      if ( key === 0) {//load header once
                          ...
                          let td_header_qty = ` <td class="table-fixed append_next">จำนวน <input type="number" width="10" class="w-50 text-right" name="topic[1][qty][${key2+1}]" value="${dtl.ins_qty}" required readonly> kg.</td>`;
          ...
          let td_result = ``;
          $.each(detail_result.material_inspect_details, function (key3, dtlrs) {
              td_result += ` <td class="append_next">
                                <input type="radio" name="times_result[1][detail][${dtlrs.ins_times}]" value='1' ${dtlrs.detail==="1"?'checked':''} ${dtlrs.detail==="1"?'checked':'disabled'} id="" required readonly> ผ่าน
                                <input type="radio" name="times_result[1][detail][${dtlrs.ins_times}]" value='0' ${dtlrs.detail==="0"?'checked':''} ${dtlrs.detail==="0"?'checked':'disabled'} id="" required readonly> ไม่ผ่าน
                            </td>`;
          });
      ...
      function constructTopic(response) {
          ...
          header_panel_time.append(td_header_time);
          let td_header_qty = ` <td class="table-fixed append_next text-center">จำนวน <input type="number" class="w-50 text-right" name="topic[1][qty][1]" width="10" required> kg.</td>`;
          header_panel_qty.append(td_header_qty);
      ...
      function appendTime(){
          ...
          header_panel_time.append(td_header_time);
          let td_header_qty = ` <td class="table-fixed append_next text-center">จำนวน <input type="number" class="w-50 text-right" name="topic[1][qty][${_time+1}]" width="10" required> kg.</td>`;
          header_panel_qty.append(td_header_qty);
          //insert result to footer
          let td_result = ` <td class="append_next">
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        if($('#formInspect').valid()){
                            $('#inspect_template_id').prop('disabled', false);
                            if($('#formInspect').submit()){
                                Swal.fire({
