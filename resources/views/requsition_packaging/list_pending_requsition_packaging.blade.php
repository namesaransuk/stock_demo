        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active">รายการรอดำเนินการเบิกบรรจุภัณฑ์</li>
                </ol>
            </div><!-- /.col -->
            <div class="col-sm-12">
                <br>
   ...
   <script>
      $(function(){
        ...
        let check_requsition_table = $('#check_requsition_table').DataTable({
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'{{route(('api.v1.requsition.packaging.pending.list'))}}',
                    'data':{
            ...
            function format ( requsition ) {
                var requsitionMaterials = requsition.packaging_cut_returns;
                var lot;
                var name_packaging;
                var total_qty;
                var coa;
                ...
                requsitionMaterials.forEach(requsition_item => {
                    ...
                    lot = requsition_item.packaging_lot.lot_no_pm;
                    name_packaging = requsition_item.packaging_lot.packaging.name;

                    total_qty = parseFloat(requsition_item.qty) ;
                    let formatter = new Intl.NumberFormat('th-TH', {currency: 'THB' });
                    let total = formatter.format(total_qty);

                    coa = requsition_item.packaging_lot.coa;
                    exp = requsition_item.packaging_lot.exp;
                    mfg = requsition_item.packaging_lot.mfg;
        ...
        $(document).on('click','.confirm-btn',function(){
                ...
                Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "post",
                                url: "{{route('api.v1.requsition.packaging.step.to.return')}}",
                                data: {'id':id},
                                dataType: "json",
                                success: function (response) {
                                    if (response) {
