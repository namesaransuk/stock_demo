    ...
    $(function() {
        ...
        let requsition_packaging_table =$('#requsition_packaging_table').DataTable({
            "pageLength": 10,
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'{{route(('api.v1.requsition.packaging.inspect.cut.list'))}}',
                    'data':{
            ...
            function format ( receive ) {
                ...
                receiveMaterials.forEach(receive_item => {
                    ...
                    total_weight = parseFloat(receive_item.qty / 1000);
                     let formatter = new Intl.NumberFormat('th-TH', {currency: 'THB' });
                     let total = formatter.format(receive_item.qty);
                ...
                table =  '<table cellpadding="5" class="w-100" cellspacing="0" border="0" style="padding-left:50px;"> รายละเอียดวัตถุดิบนำเข้า'+inner_table+'</table>';
            ...
            $(document).on('click','.confirm-btn',function(){
                ...
                Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "post",
                                url: "{{route('api.v1.requsition.packaging.step.to.pending')}}",
                                data: formData,
                                contentType: false,
                                processData: false,
                                cache: false,
            ...
            $(document).on('click','.cancel-btn',function(){
                ...
                Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "post",
                                url: "{{route('api.v1.requsition.packaging.step.back.to.cut')}}",
                                data: formData,
                                contentType: false,
                                processData: false,
                                cache: false,
