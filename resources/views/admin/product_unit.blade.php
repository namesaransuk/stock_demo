        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active">หน่วยนับของสินค้า</li>
                </ol>
            </div><!-- /.col -->
            <div>
                <br>
        ...
        <div class="col-12">
            <div class="card card-outline card-gdp">
                <div class="card-header ogn-stock-yellow text-left" >
                    <h6 class="m-0">หน่วยนับของสินค้า</h6>
                </div>
                <div class="p-2">
                ...
                <div class="card-body p-0">
                    ...
                    <table id="product_unit_table" class="table w-100" style="margin-top: 0!important; margin-bottom: 0!important;">
  ...
  <div class="modal-dialog ">
    <div class="modal-content">
      <div class="modal-header ogn-stock-green">
        <h5 class="modal-title" id="staticBackdropLabel" style="color: white;">เพิ่มรายการหน่วยนับของสินค้า</h5>
...
@section('js')
    <script>
        ...
        let product_unit_table = $('#product_unit_table').DataTable({
            "pageLength": 10,
                "responsive": true,
                // "order": [4, "desc"],
                'processing': true,
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'{{route(('api.v1.product.unit.list'))}}'
                },
                'columns': [
                    ...
                    { data: 'name' },
                    { data: 'id', render : function(data, type, row, meta){
                            return "บรรจุ "+ row.multiply + " ชิ้น";
                        } },
                    { data: 'id',
        ...
        $('#custom-search-input').keyup(function(){
            product_unit_table.search($(this).val()).draw() ;
        })
        ...
        $(document).on('click','.btn-modal',function(){
            ...
            if(!id){
                ...
                $('.modal-title').text('เพิ่มรายการหน่วยนับของสินค้า');
                $('#name').val('');
                $('#multiply').val('');
                $('#id').val('');
                ...
                $('.modal-title').text('แก้ไขรายการหน่วยนับของสินค้า');
                $.ajax({
                    type: "post",
                    url: "{{route(('api.v1.product.unit.view.edit'))}}",
                    data: {'id':id},
                    dataType: "json",
        ...
        $('.btn-save').on('click', function() {
            ...
            if (!formData.id) {
                $.ajax({
                type: "post",
                url: "{{route(('api.v1.product.unit.validate'))}}",
                data: formData,
                dataType: "json",
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        Swal.fire({
                            title: 'ยืนยันการเพิ่มหน่วยนับของสินค้า?',
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    type: "post",
                                    url: "{{route(('api.v1.product.unit.create'))}}",
                                    data: formData,
                                    dataType: "json",
                                    success: function(response) {
                        ...
                        if ($('#multiply').val() === '') {
                            ...
                            $('#checkmultiply').addClass('text-danger').text('กรุณากรอกข้อมูล');
                        } else {
                            $('#checkmultiply').hide()
                        }
                    }
                ...
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        Swal.fire({
                            title: 'ยืนยันการแก้ไขหน่วยนับของสินค้า?',
                            text: "ต้องการดำเนินการใช่หรือไม่!",
                            icon: 'warning',
                            showCancelButton: true,
                            confirmButtonColor: '#649514',
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    type: "post",
                                    url: "{{route(('api.v1.product.unit.edit'))}}",
                                    data: formData,
                                    dataType: "json",
                                    success: function(response) {
        ...
        $(document).on('click','.btn-delete',function(){
            let id = $(this).data('id');
            Swal.fire({
                title: 'ยืนยันการลบหน่วยนับของสินค้า?',
                text: "ต้องการดำเนินการใช่หรือไม่!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#649514',
                ...
                cancelButtonText: 'ปิด'
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "post",
                                url: "{{route('api.v1.product.unit.delete')}}",
                                data: {'id':id},
                                dataType: "json",
                                success: function (response) {
                                    if (response) {
