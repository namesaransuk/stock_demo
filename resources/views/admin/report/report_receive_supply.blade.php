    ...
    <div class="row mb-2">
        <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">รายงานการรับเข้าวัสดุสิ้นเปลือง</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
...
<div class="row">
    <div class="col-12">
        <div class="card card-outline card-gdp">
            ...
            <form action="{{route('report.receive.supply.pdf')}}" method="POST">
                @csrf
...
<script>
    ...
    function showTable() {
        ...
        $('#requsition_supply_table').DataTable({
                ...
                'scrollX': true,
                'ajax': {
                    'url':'{{route(('api.v1.report.receive.supply.list'))}}',
                    'data':{
                ...
                'columns': [
                    { data: 'id', render : function(data, type, row, meta){
                        ...
                        if(row.receive_supply.paper_status == 3){
                            text = '<span class="badge bg-success text-dark">รับเข้าสำเร็จ</span>';
                        }
                        // if(row.receivesupply.reject_status == 1){
                    ...
                    { data: 'id', render : function(data, type, row, meta){
                        if (row.receive_supply.brand_vendor.brand) {
                            return row.receive_supply.brand_vendor.brand;
                        }else{
                            return 'ไม่มีข้อมูล'
                        }
                    } },
                    { data: 'id', render : function(data, type, row, meta){
                        if (row.receive_supply.stock_user.employee.fname) {
                            return row.receive_supply.stock_user.employee.fname;
                        }
                    } },
                    { data: 'id', render : function(data, type, row, meta){
                        if (row.receive_supply.audit_user.employee.fname) {
                            return row.receive_supply.audit_user.employee.fname;
                        }
                    } },
                ],
                columnDefs: [
