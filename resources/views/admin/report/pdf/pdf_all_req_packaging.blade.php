        ...
        <table class="border_table">
            <thead>
                <tr>
                    ...
                    <th class="border_table" style="width:900px"><h1>บันทึกการเบิกบรรจุภัณฑ์</h1>  </th>
                </tr>
            </thead>
        </table>
            ...
            <tbody style="text-align: center">
                ...
                @foreach ($data as $data_one)
                    ...
                    <tr>
                        ...
                        <td class="border_table">{{$data_one->packagingLot->lot}}</td>
                        <td class="border_table">{{number_format($data_one->qty)}} ชิ้น</td>
                        @if ($data_one->requsitionPackaging->productionUser)
                            <td class="border_table">{{$data_one->requsitionPackaging->productionUser->employee->fname}}</td>
                        @else
                            <td class="border_table">ไม่มีข้อมูล</td>
                        @endif
                        @if ($data_one->requsitionPackaging->procurementUser)
                            <td class="border_table">{{$data_one->requsitionPackaging->procurementUser->employee->fname}}</td>
                        @else
                            <td class="border_table">ไม่มีข้อมูล</td>
                        @endif
                        <td class="border_table">{{$data_one->requsitionPackaging->recap}}</td>
