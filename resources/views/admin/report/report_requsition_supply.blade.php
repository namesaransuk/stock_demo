    ...
    <div class="row mb-2">
        <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">รายงานการเบิกวัสดุสิ้นเปลือง</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
...
<div class="row">
    <div class="col-12">
        <div class="card card-outline card-gdp">
            <form action="{{route('report.requsition.supply.pdf')}}" method="POST">
                @csrf
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-3">
                        <div class="form-group">
                            <label for="item_search">วัสดุสิ้นเปลือง</label>
                            <select class="form-control form-control-sm select2" id="item_search" name="item_search">
                                ...
                                @foreach ($sup as $sup_one)
                                    <option data-id="{{$sup_one->id}}" value="{{$sup_one->id}}">{{$sup_one->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<script>
    ...
    function showTable() {
        ...
        $('#requsition_supply_table').DataTable({
                ...
                'scrollX': true,
                'ajax': {
                    'url':'{{route(('api.v1.report.requsition.supply.list'))}}',
                    'data':{
                ...
                'columns': [
                    { data: 'id', render : function(data, type, row, meta){
                            text = '<span class="badge bg-warning text-dark w-100">กำลังดำเนินการ</span>';
                        if(row.action == 1){
                            text = '<span class="badge bg-success text-dark w-100">เบิกสำเร็จ</span>';
                        }
                        return text;
                    } },
                    ...
                    { data: 'id', render : function(data, type, row, meta){
                        if (row.supply_lot.lot) {
                            return row.supply_lot.lot;
                        }else{
                            return 'ไม่มีข้อมูล'
                        }
                    } },
                    { data: 'id', render : function(data, type, row, meta){
                        if (row.supply_lot.supply.name) {
                            return row.supply_lot.supply.name;
                        }else{
                            return 'ไม่มีข้อมูล'
                        }
                    } },
                    ...
                    { data: 'id', render : function(data, type, row, meta){
                        if (row.requsition_supply.stock_user.employee.fname) {
                            return row.requsition_supply.stock_user.employee.fname;
                        }
                    } },
                    { data: 'id', render : function(data, type, row, meta){
                            text = 'ไม่มีข้อมูล';
                        if(row.requsition_supply.recap){
                            text = row.requsition_supply.recap;
                        }
                        return text;
                    } },
                ],
