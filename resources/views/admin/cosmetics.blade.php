        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active">ประเภทเครื่องสำอาง</li>
                </ol>
            </div><!-- /.col -->
            <div>
                <br>
        ...
        <div class="col-12">
            <div class="card card-outline card-gdp">
                <div class="card-header ogn-stock-yellow text-left">
                    <h6 class="m-0"> ประเภทเครื่องสำอาง</h6>
                ...
                <div class="card-body p-0">
                    ...
                    <table id="cosmetics_table" class="table w-100"
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header ogn-stock-green">
                    <h5 class="modal-title" id="staticBackdropLabel" style="color: white;">เพิ่มประเภทเครื่องสำอาง</h5>
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            let cosmetics_table = $('#cosmetics_table').DataTable({
                "pageLength": 10,
                "responsive": true,
                // "order": [4, "desc"],
                'processing': true,
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url': '{{ route('api.v1.cosmetics.list') }}'
                },
                'columns': [{
                        data: 'id',
            ...
            $('#custom-search-input').keyup(function() {
                cosmetics_table.search($(this).val()).draw();
            })
            ...
            $(document).on('click', '.btn-modal', function() {
                ...
                if (!id) {
                    ...
                    $.ajax({
                        type: "post",
                        url: "{{ route('api.v1.cosmetics.view.edit') }}",
                        data: {
                            'id': id
                        },
                        dataType: "json",
            ...
            $(document).on('click', '.btn-save', function() {
                ...
                if (!formData.id) {
                    $.ajax({
                        type: "post",
                        url: "{{ route('api.v1.cosmetics.validate') }}",
                        data: formData,
                        dataType: "json",
                        success: function(data) {
                            if ($.isEmptyObject(data.error)) {
                                Swal.fire({
                                    title: 'ยืนยันการเพิ่มประเภทเครื่องสำอาง?',
                                    ...
                                    if (result.isConfirmed) {
                                        $.ajax({
                                            type: "post",
                                            url: "{{ route('api.v1.cosmetics.create') }}",
                                            data: formData,
                                            dataType: "json",
                                            success: function(response) {
                                                if (response) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if ($.isEmptyObject(data.error)) {
                                Swal.fire({
                                    title: 'ยืนยันการแก้ไขประเภทเครื่องสำอาง?',
                                    text: "ต้องการดำเนินการใช่หรือไม่!",
                                    icon: 'warning',
                                    showCancelButton: true,
                                    confirmButtonColor: '#649514',
                                    ...
                                    if (result.isConfirmed) {
                                        $.ajax({
                                            type: "post",
                                            url: "{{ route('api.v1.cosmetics.edit') }}",
                                            data: formData,
                                            dataType: "json",
                                            success: function(response) {
                                                if (response) {
            ...
            $(document).on('click', '.btn-delete', function() {
                let id = $(this).data('id');
                Swal.fire({
                    title: 'ยืนยันการลบประเภทเครื่องสำอาง?',
                    text: "ต้องการดำเนินการใช่หรือไม่!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#649514',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "post",
                            url: "{{ route('api.v1.cosmetics.delete') }}",
                            data: {
                                'id': id
                            },
                            dataType: "json",
