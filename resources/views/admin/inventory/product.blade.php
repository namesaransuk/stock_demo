    ...
    <div class="row mb-2">
        <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">สินค้าคงคลัง</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
    ...
    <div class="col-12">
        <div class="card card-outline card-gdp mt-2">
            <div class="card-header ogn-stock-yellow text-left" >
                <h6 class="m-0"> สินค้า </h6>
                <input type="hidden" name="company_id" id="company_id" value="{{session('company')}}">
            </div>

            {{-- <div class="card-body p-3">
                <canvas id="productBarChart" style="min-height: 250px; height: 250px; max-height: 250px; max-width: 100%;"></canvas>
            </div> --}}
        ...
        {{-- <div class="text-right float-right mb-3">
            <a type="submit" href="{{ route('product.export') }}"
...
<script>
    ...
    function addChart() {
        $.ajax({
                    url: "{{route('inventory.product.chart')}}",
                    type:"POST",
                    data:{
                        year: 2021,
                    },
                    success:function(data)
                    {
                        ...
                        // chart
                        var supplyChartCanvas = $('#productBarChart').get(0).getContext('2d')
                        var supplyData = {
                            labels: data.lable,
                            datasets: [{
                                // label: '# of Votes',
                        ...
                        productBarChart = new Chart(supplyChartCanvas, {
                            type: 'bar',
                            data: supplyData,
    ...
    function showTable() {
        ...
        $('#requsition_supply_table').DataTable({
            "pageLength": 10,
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'{{route(('api.v1.inventory.product.list'))}}',
                    'data':{
