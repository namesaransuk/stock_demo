        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active">วัสดุสิ้นเปลือง</li>
                </ol>
            </div><!-- /.col -->
            <div>
                <br>
        ...
        <div class="col-12">
            <div class="card card-outline card-gdp">
                ...
                <div class="card-body p-0">
                    ...
                    <table id="supply_table" class="table w-100" style="margin-top: 0!important; margin-bottom: 0!important;">
  ...
  <div class="modal-dialog ">
    <div class="modal-content">
      <div class="modal-header" style="background-color: #9BC56D;">
        <h5 class="modal-title" id="staticBackdropLabel" style="color: white;">เพิ่มรายการวัสดุสิ้นเปลือง</h5>
        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 ">
                    <label for="" class="form-label">วัสดุสิ้นเปลือง</label>
...
@section('js')
    <script>
        ...
        let supply_table = $('#supply_table').DataTable({
            "pageLength": 10,
                "responsive": true,
                // "order": [4, "desc"],
                'processing': true,
                ...
                'columns': [
                    ...
                    render : function(data,type,row,meta){
                        return `<div class="dropdown show">
                                    <a class="btn btn-sm text-secondary rounded border ml-1" href="#" supply="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fas fa-edit"></i> จัดการ</a>
        ...
        $('#custom-search-input').keyup(function(){
            supply_table.search($(this).val()).draw() ;
            })
        ...
        $(document).on('click','.btn-modal',function(){
            ...
            if(!id){
                ...
                $('.modal-title').text('เพิ่มรายการวัสดุสิ้นเปลือง');
                $('#name').val('');
                $('#id').val('');

            } else {

                $('.modal-title').text('แก้ไขรายการวัสดุสิ้นเปลือง');
                $.ajax({
                    type: "post",
                    url: "{{route(('api.v1.supply.view.edit'))}}",
                    data: {'id':id},
                    dataType: "json",
        ...
        $('.btn-save').on('click', function() {
            ...
            if (!formData.id) {
                $.ajax({
                type: "post",
                url: "{{route(('api.v1.supply.validate'))}}",
                data: formData,
                dataType: "json",
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        Swal.fire({
                            title: 'ยืนยันการเพิ่มวัสดุสิ้นเปลือง?',
                            ...
                            if (result.isConfirmed) {
                                $.ajax({
                                    type: "post",
                                    url: "{{route(('api.v1.supply.create'))}}",
                                    data: formData,
                                    dataType: "json",
                                    success: function(response) {
                ...
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        Swal.fire({
                            title: 'ยืนยันการแก้ไขวัสดุสิ้นเปลือง?',
                            text: "ต้องการดำเนินการใช่หรือไม่!",
                            icon: 'warning',
                            showCancelButton: true,
                            confirmButtonColor: '#649514',
        ...
        $(document).on('click','.btn-delete',function(){
            let id = $(this).data('id');
            Swal.fire({
                title: 'ยืนยันการลบวัสดุสิ้นเปลือง?',
                text: "ต้องการดำเนินการใช่หรือไม่!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#649514',
                ...
                cancelButtonText: 'ปิด'
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "post",
                                url: "{{route('api.v1.supply.delete')}}",
                                data: {'id':id},
                                dataType: "json",
                                success: function (response) {
                                    if (response) {
