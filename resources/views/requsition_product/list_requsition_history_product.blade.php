    ...
    <div class="row mb-2">
        <div class="col-sm-12">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">ประวัติการส่งออก</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
    ...
    <div class="col-12">
        <div class="card card-outline card-gdp">
            <div class="card-body p-0">
                <div class="position-absolute m-2 text-md">ค้นหาจาก หมายเลขเอกสาร รายละเอียด</div>
    ...
    $(function() {
        ...
        let requsition_product_table =$('#requsition_product_table').DataTable({
            "pageLength": 10,
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'{{route(('api.v1.requsition.product.history.list'))}}',
                    'data':{
                ...
                'columns': [
                    ...
                    },
                    {
                        ...
                        render : function(data, type, row, meta){
                            var status = "";
                                status = `<span class='badge badge-pill w-75 text-success' style='font-size:15px;'>ส่งออกสำเร็จ</span>`
                            return status;
                        }
                    },
                    { data: 'id',render : function(data,type,row,meta){
                            ref = "{{url('/report/requsition/product/pdf/')}}";
            ...
            $(document).on('click','.confirm-btn',function(){
                ...
                Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                type: "post",
                                url: "{{route('api.v1.requsition.product.step.to.history')}}",
                                data: {'id':id},
                                dataType: "json",
