    ...
    $(function() {
        ...
        let receive_product_table =$('#receive_product_table').DataTable({
            "pageLength": 10,
                ...
                'serverMethod': 'post',
                'ajax': {
                    'url':'{{route(('api.v1.receive.product.list'))}}',
                    'data':{
                ...
                'columns': [
                    ...
                    render : function(data,type,row,meta){
                        let url2 = "{{ route('receive.product.edit', "__id") }}".replaceAll("__id",row.id);
                        let url3 = "{{ route('receive.product.history', "__id") }}".replaceAll("__id",row.id);
                        if (row.edit_times != 0) {
                        ...
                        apply_next = `<a type="button"  style=""  id="myModal-edit" data-id="${row.id}"  class= "reset-me btn-modal btn-update-inspect_ready " data-toggle="tooltip" data-placement="top" title="ส่งต่อเพื่อตรวจสอบ" ><i class="fas fa-arrow-circle-right p-1 text-success"></i> </a>`;
            ...
            function format ( receive ) {
                ...
                var receiveprodcuts = receive.product_lots;
                var lot;
                var name_product;
                var total_weight;
                var coa;
                ...
                receiveprodcuts.forEach(receive_item => {
                    ...
                    loop_table =    '<tr>'+
                                        ...
                                        '<td>'+lot+'</td>'+
                                        '<td>'+name_product+'</td>'+
                                        '<td class="text-right">'+qty+' ชิ้น'+'</td>'+
            ...
            Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "post",
                            url: "{{route('api.v1.receive.product.edit.inspect.ready')}}",
                            data: {'id':id},
                            dataType: "json",
            ...
            Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "post",
                            url: "{{route('api.v1.receive.product.delete')}}",
                            data: {'id':id},
                            dataType: "json",
                            success: function (response) {
                                if (response) {
