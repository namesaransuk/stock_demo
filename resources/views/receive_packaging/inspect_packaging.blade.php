        ...
        <div class="row mb-2">
            <div class="col-sm-12">
                <ol class="breadcrumb float-sm-left">
                    ...
                    <li class="breadcrumb-item active"> <a href="{{ route('check.import.packaging.stock') }}">ตรวจสอบรายการนำเข้าบรรจุภัณฑ์</a> </li>
                    <li class="breadcrumb-item active">ตรวจสอบบรรจุภัณฑ์</li>
                </ol>
            </div><!-- /.col -->
...
@section('content')
    <div class="card">
        ...
        <form id="formInspect" action="{{route('receive.packaging.check.save')}}" method="POST">
            @csrf
        <div class="card-body">
            <div class="row">
                <div class="col-sm-4">
                    <input type="hidden" name="id" id="id" value="{{$pac_lot_detail->id}}">
                ...
                <div class="col-sm-4 pt-2">
                    ...
                    <input class="form-control" type="text" name="" id="" value="{{$pac_lot_detail->qty}}" readonly>
                </div>

                <div class="col-sm-2 pt-2">
                    <label class="label-control" for="">MFG.</label>
                    <input class="form-control" type="text" name="" id="" value="{{$pac_lot_detail->mfg}}" readonly>
                </div>
                <div class="col-sm-2 pt-2">
                    <label class="label-control" for="">EXP.</label>
                    <input class="form-control" type="text" name="" id="" value="{{$pac_lot_detail->exp}}" readonly>
                </div>
...
@section('js')
   <script>
      ...
      $(function(){
        ...
        var packaging_lot_id = $('#id').val();
        $.ajax({
            type: "post",
            url: "{{route('api.v1.receive.packaging.quality.check.detail')}}",
            data: {"id" : packaging_lot_id},
            dataType: "JSON",
        ...
        let check_receive_table = $('#receive_packaging_table_check').DataTable({
            fixedColumns: {
                leftColumns: 2
            },
            scrollY:        300,
        ...
        $('#receive_packaging_table_check tbody').on('click', '.list_cancel', function() {
        ...
        $('#inspect_template_id').on('change',function(){
            ...
            let in_html = ` <div class="row mb-2">

                                </div> `
            $.ajax({
                type: "post",
                url: "{{route('api.v1.receive.packaging.get.template.detail')}}",
        ...
        //   console.log(response)
          ...
          $.each(response, function(key, topic){
              if ( topic.ins_type==="1") {
                  let tds = ``;
                  // console.log(topic.)
                  let _detail_len = topic.packaging_inspect_details.length-1;
                  $.each(topic.packaging_inspect_details, function (key2, dtl) {
          ...
          let td_result = ``;
          $.each(detail_result.packaging_inspect_details, function (key3, dtlrs) {
